      ...
      <div className="space-y-6">
        <div className="flex gap-2">
          <button
            ...
            className={`text-[13px] font-medium px-4 py-2 rounded-full transition-colors ${
              aba === "recentes" ? "bg-[#E63A1F] text-white" : "bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-300"
            }`}
          >
            Recentes
          </button>
          <button
            ...
            className={`text-[13px] font-medium px-4 py-2 rounded-full transition-colors ${
              aba === "regras" ? "bg-[#E63A1F] text-white" : "bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-300"
            }`}
          >
            Regras cadastradas
          </button>
        ...
        {!loading && aba === "recentes" && (
          <div className="space-y-2">
            ...
            {alertas.map((a) => (
              <div key={a.id} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4 flex items-start gap-3">
                <span
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className="text-[11px] text-zinc-400 dark:text-zinc-500">{a.regra.evento}</span>
                  </div>
                  <p className="text-[13px] text-zinc-700 dark:text-zinc-300">{a.payload.mensagem}</p>
                </div>
                <span className="text-[11px] text-zinc-400 dark:text-zinc-500 font-mono tabular-nums whitespace-nowrap">
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-zinc-100 dark:border-zinc-800 text-left">
                  <th className="px-5 py-3 font-medium text-zinc-500 dark:text-zinc-400 text-[11px] uppercase tracking-wide">Categoria</th>
                  <th className="px-5 py-3 font-medium text-zinc-500 dark:text-zinc-400 text-[11px] uppercase tracking-wide">Evento</th>
                  <th className="px-5 py-3 font-medium text-zinc-500 dark:text-zinc-400 text-[11px] uppercase tracking-wide">Descrição</th>
                  <th className="px-5 py-3 font-medium text-zinc-500 dark:text-zinc-400 text-[11px] uppercase tracking-wide">Canal</th>
                  <th className="px-5 py-3 font-medium text-zinc-500 dark:text-zinc-400 text-[11px] uppercase tracking-wide text-right">Ativa</th>
                </tr>
              </thead>
              <tbody>
                {regras.length === 0 && (
                ...
                {regras.map((r) => (
                  <tr key={r.id} className="border-b border-zinc-50 dark:border-zinc-800/50 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-800/60/50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 font-mono text-[12px] text-zinc-600 dark:text-zinc-300">{r.evento}</td>
                    <td className="px-5 py-3.5 text-zinc-700 dark:text-zinc-300">{r.descricao}</td>
                    <td className="px-5 py-3.5 text-[12px] text-zinc-500 dark:text-zinc-400">{r.canal}</td>
                    <td className="px-5 py-3.5 text-right">
                      ...
                      >
                        <span
                          className={`absolute top-0.5 h-4 w-4 rounded-full bg-white dark:bg-zinc-900 transition-transform ${
